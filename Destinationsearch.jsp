...
<title>Train Schedule Determined By Station of Destination</title>
</head>
<body>
	<%
		...
		try {
			...
			String entity = request.getParameter("destination");
			String entity2=request.getParameter("order by");
			//Make a SELECT query from the sells table with the price range specified by the 'price' parameter at the index.jsp
			String str = "Select ss.transit_line, ss.trainid, ss.origin, rs.departure_date, rs.departure_time, s.name, ss.date_arrival_time, rs.destination, rs.arrival_date, rs.arrival_time From Station_stops ss, Reservation_Schedule rs, Station s where ss.unique_id=s.unique_id and ss.transit_line=rs.transit_line and ss.trainid=rs.trainid and ss.origin=rs.origin and ss.trainid in(Select rs.trainid From Reservation_Schedule rs Where rs.destination='"+entity+"')order by rs."+entity2+", rs.trainid, ss.date_arrival_time";
